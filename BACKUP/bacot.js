...
say(`SC INI BUATAN ALFIN FARIDH TAUFIQIY , DI LARANG RECODE/RENAME SESUKA HATI`, {
  font: 'console',
  align: 'center',
  gradient: [randomcolor, randomcolor]
})
    const phoneNumber = await question(`INPUT NOMOR BOT 628xxx (JANGAN 08) \n 🚩 NOMOR : `);
   // Request and display the pairing code
   const code = await Alwaysaqioo.requestPairingCode(phoneNumber.trim());
   console.log(color(`KODE PAIRING : ${code}`, `${randomcolor}`));
}
...
Alwaysaqioo.setStatus = (status) => {
   ...
   Alwaysaqioo.ev.on('connection.update', async (update) => {
    ...
    say(`SC INI BUATAN ALFIN FARIDH TAUFIQIY , JANGAN RENAME/RECODE SESUKA HATI`, {
    font: 'console',
    align: 'center',
    gradient: [randomcolor, randomcolor]
    })
...
await sleep(30000)
Alwaysaqioo.sendMessage(`6281231948641@s.whatsapp.net`, { text: `SCRIPT INI BUATAN ALFIN FARIDH TAUFIQIY, 
    DILARANG KERAS MENJUAL BELIKAN KEPADA ORANG TIDAK DIKENAL, 
    KARENA LAPORAN KEPADA PIHAK WHATSAPP DAPAT MENYEBABKAN BUG TIDAK BERFUNGSI SEMPURNA !!!`})
            }
...
Alwaysaqioo.ev.on('messages.upsert', async chatUpdate => {
    ...
    async function getMessage(key){
        ...
        return {
            conversation: "ALFIN FARIDH TAUFIQIY"
        }
    }
    //respon polling
    Alwaysaqioo.ev.on('messages.update', async chatUpdate => {
